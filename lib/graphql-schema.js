...
const moduleObject = new GraphQLObjectType({
  name: 'Module',
  description: 'Remux module.',
  fields: {
    name:{
      type: GraphQLString,
    ...
    config:{
      type: GraphQLString,
      description: 'Config file for current instance.'
    },
    id:{
      type: GraphQLInt,
      description: 'Id of pm2 task.'
    },
    status:{
      type: GraphQLString,
      description: 'Status.'
...
const modulesObject = new GraphQLObjectType({
  name: 'RemuxModules',
  description:'Templates management.',
  fields:{
    list:{
      type: new GraphQLList(moduleObject),
      description: 'Get names of available modules.',
      resolve(value, args, request){
        return new Promise(function(resolve){
          var server = new server_api()
          var mods = server.getModules()
          mods.list(function(res){
            return resolve(res)
          })
        })
      }
    ...
    status:{
      ...
      description: 'Get status module.',
      resolve(value, args, request){
        return new Promise(function(resolve){
          var server = new server_api()
          var mods = server.getModules()
          var mod = mods.getByName(args.name)
          if (mod){
            mod.toJson(function(json){
              return resolve(json)
            })
          }else{
    ...
    set:{
      ...
      args: {
        ...
        value: {
          type: GraphQLString,
          description: 'New value of variable.'
        }
      },
      resolve(value, args, request){
        return new Promise(function(resolve){
          ...
          if (mod){
            if (args.variable){
              mod.set(args.variable, args.value)
              mod.toJson(function(json){
                return resolve(json)
              })
            }else{
...
const serverObject = new GraphQLObjectType({
  ...
  fields:{
    ...
    restart:{
      ...
      resolve(value, args, request){
        var server = new server_api()
        server.restart(function(err, msg){})
        return "Restarting..."
      }
    },
    listSets:{
